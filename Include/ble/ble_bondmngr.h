 ...
 *
 * @details When using <i>Bluetooth</i> low energy, a master device and a slave device can
 ...
 *          bond or to use a previous bond when it reconnects to a bonded master. It is however up
 ...
 *          from a previously bonded master. This identification information is referred to as
 ...
 *          to a connection when bonding with a master. On reconnection with the known bonded
 *          master, the application restores the related persistent attributes in the last known
 ...
 *          application when it's connected to a new bonded master or to a previously bonded master.
 *          The application can use the Bond Manager API to store or load (or restore) the
 *          Bonding Information and System Attributes. The bond manager identifies all the masters
 ...
 *          
 *          - To a given bonded master using directed advertisement.
 *          - To a list of bonded masters - i.e. using whitelist.
 *
 *          The bond manager automatically writes the Bonding Information to the flash memory when
 *          the bonding procedure to a new master is finished. Upon disconnection, the application
 *          should use the Bond Manager API @ref ble_bondmngr_bonded_masters_store to write the
 ...
 *          data for the current connected master is already present in the flash. The API does so
 ...
 *          
 *          Refer to @ref ble_bond_mgr_msc to see the flow of events when connecting to a master
 ...
 * @{
 * @defgroup UNBONDED_MASTER Connecting to an unbonded master
 * @image html bond_manager_unbonded_master.jpg Connecting to an unbonded master
 * @defgroup BONDED_MASTER Connecting to a bonded master
 * @image html bond_manager_bonded_master.jpg Connecting to a bonded master
 * @}
 * @}
 */
...
typedef struct
{
    ...
    int8_t                  master_handle;                      /**< Handle to the current master. This is an index to the master list maintained internally by the bond manager. */
    uint16_t                master_id;                          /**< Identifier to the master. This will be the same as Encryption diversifier of the master (see @ref ble_gap_evt_sec_info_request_t). This value is constant for the duration of the bond. */
} ble_bondmngr_evt_t;
...
typedef struct
{
    ...
    bool                        bonds_delete;                   /**< TRUE if bonding and System Attributes for all masters is to be deleted from flash during initialization, FALSE otherwise. */
...
/**@brief      Handles all events from the BLE stack that relate to this module.
 *
 * @param[in]  p_ble_evt  The event received from the BLE stack.
 *
 * @return     NRF_SUCCESS if all operations went successfully,
 *             NRF_ERROR_NO_MEM if the maximum number of bonded masters has been reached.
 ...
 *
 * @note        This function returns NRF_ERROR_INVALID_PARAM if the given master has a private
 *              address.
 *
...
/**@brief   This function stores the System Attributes of a newly connected master.
 *
 * @details This function fetches the System Attributes of the current master from the stack, adds
...
/**@brief           Function to fetch the identifiers of known masters.
 *
 * @details         This function fetches the identifiers of the masters that are currently in the
 *                  database, or in other words, known to the bond manager.
 *
 * @param[out]      p_master_ids    Pointer to the array of master identifiers. It is recommended 
 *                                  that the length of this array be equal to 
 *                                  MAX_NUMBER_OF_BONDED_MASTERS * 2 bytes. If value of this pointer
 *                                  is NULL, only the number of masters in the database will be
 *                                  filled in p_length. This can be used to find out the
 *                                  required size of the array pointed to by p_master_ids in
 *                                  a subsequent call. 
 * @param[in, out]  p_length        Pointer to the length of p_master_ids array provided as
 *                                  input. On return, this function will write the number of master 
 ...
 *                  p_master_ids provided is not enough to fit in all the master identifiers in the
 *                  database.
 */
uint32_t ble_bondmngr_master_ids_get(uint16_t * p_master_ids, uint16_t * p_length);

/**@brief       Function to delete a single master from the database.
 * @details     This function deletes the Bonding Information and System Attributes of a single
 *              master from the flash.
 *              The application can use the @ref ble_bondmngr_master_ids_get function to fetch the
 *              identifiers of masters present in the database and then call this function.
 *
 * @warning     This function could prevent the radio from running. Therefore it MUST be called ONLY
 ...
 *              undefined.</b>
 *
 * @param[in]   master_id   Identifier of the master to be deleted.
 *
 * @return      NRF_SUCCESS on success.
 *              NRF_ERROR_INVALID_STATE is returned if the bond manager was not initialized.
 *              NRF_ERROR_NOT_FOUND if the master with the given identifier is not found in the
 *              database.
 */
